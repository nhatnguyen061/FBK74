            ...
            <div class="container-fluid">
                <div class="row">
                    ...
                    <div class="col-md-10 mx-auto">
                        <h2 class="registered-tournaments-title text-center my-4">GIẢI ĐẤU ĐÃ ĐĂNG KÝ</h2>
                        
                        <div class="tournament-list">
                            <c:forEach items="${tournaments}" var="tournament">
                                <div class="tournament-item d-flex align-items-center border rounded p-3 mb-4 bg-light">
                                    <!-- Hình ảnh giải đấu -->
                                    <div class="tournament-image-wrapper mr-3">
                                        <img src="img_nam/${tournament.image}" alt="${tournament.name}" class="tournament-image img-fluid" style="max-width: 100px; max-height: 100px;">
                                    </div>
                                    
                                    <!-- Thông tin giải đấu -->
                                    <div class="tournament-info flex-grow-1">
                                        <a href="./TournamentDetail?tournamentID=${tournament.id}&isFromHistory=isFromHistory" class="animate__animated animate__zoomIn text-dark">
                                            <h5 class="tournament-name font-weight-bold mb-2">${tournament.name}</h5>
                                        </a>
                                        
                                        <!-- Nút chọn giải đấu -->
                                        <button class="btn btn-primary select-button mr-2" onclick="toggleHighlight(this)">Chọn</button>
            
                                        <!-- Điều kiện để hủy giải đấu -->
                                        <c:if test="${tournament.getStart().after(currentDate)}">
                                            <a href="./CancelRegisTournament?tournamentID=${tournament.id}" class="btn btn-danger cancel-registration" 
                                               onclick="return confirm('Bạn có chắc chắn muốn hủy giải đấu? Bạn cần 1 thời gian để admin xem xét có hủy được hay không. Và đội bóng của bạn vẫn đang được đăng ký trong thời gian xem xét !')">Hủy giải đấu đã đăng ký</a>
                                            <p class="text-muted small mt-2">(Lưu ý: Chỉ được hủy những giải đấu đang trong thời gian mở đơn đăng ký)</p>
                                        </c:if>
                                    </div>
                                </div>
                            </c:forEach>
