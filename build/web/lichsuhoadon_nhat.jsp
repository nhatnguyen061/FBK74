...
<html>
    ...
    <body>
        ...
        <div class="container mt-4 min-vh-100">
            <h2 class="mb-4 text-center">Thông Tin Hóa Đơn</h2>
            <table class="table table-bordered">
                ...
                <tbody>
                    <c:forEach var="list" items="${requestScope.listBill}">
                        <tr id="${list.IDBill}">
                            <td>${list.paymentDate}</td>
                            <td>${list.invoice}</td>
                            <c:if test="${not list.invoice.contains('A') && list.registFindOpponent!=null}">
                                <td>Cọc ĐK đối</td>
                            </c:if>
                            <c:if test="${not list.invoice.contains('A') && list.registFindOpponent==null}">
                                <td>Cọc Sân</td>
                            </c:if>
                            <c:if test="${list.invoice.contains('A')}">
                                <td>Thanh toán sân</td>
                            </c:if>
                            <c:if test="${not list.invoice.contains('A')}">
                                <td>100000 VNĐ</td>
                            </c:if>
                            <c:if test="${list.invoice.contains('A')}">
                                <td>${list.totalPrice} VNĐ</td>
                            </c:if>
                            <!--check xem nếu là đơn cọc thì đã hoàn hãy thanh toán và tiền hóa đơn thì luôn thanh toán-->
                            <c:choose>
                                <c:when test="${list.registeredFootballField.status==5 && list.registFindOpponent==null}">
                                    <td>Đã hoàn tiền</td>
                                </c:when>
                                <c:when test="${list.registeredFootballField==null && list.registFindOpponent.status==5}">
                                    <td>Đã hoàn tiền</td>
                                </c:when>
                                <c:otherwise>
                                    <td>Đã thanh toán</td>
                            ...
                            <td><a href="chitiethoadon_cus?idBill=${list.IDBill}" class="btn btn-info">Chi tiết hóa đơn</a></td>
                        </tr>
                    </c:forEach>
